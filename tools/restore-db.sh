...
docker-compose exec -T pgmaster pg_dump ${src_psql_uri} > ${filename} &
pid=$!
...
docker-compose exec pgmaster psql $dest_psql_uri -c 'DROP SCHEMA PUBLIC CASCADE; DROP SCHEMA postgraphql CASCADE; CREATE SCHEMA PUBLIC; CREATE SCHEMA postgraphql;'
echo "${dark_gray}Database dropped.\n"

echo "${green}[06/08]${nc}\t${light_gray}Restoring dump file into local database...${nc}"
docker exec -i bondeinstall_pgmaster_1 psql $dest_psql_uri < $filename_owner_changed
